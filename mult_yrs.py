...
yr_elev_seq = np.array([0, 10, 20])  # Dynamic timeline evaluation sequence
...
# --- 2. Evaluate Strategies & Timelines ---
# Multi-dimensional allocation maps: (Strategy, Year, SOW)
led_ens = np.zeros((num_strat, num_years, nsow))   
cc_ens = np.zeros((num_strat, num_years, nsow))    
lr_ens = np.zeros((num_strat, num_years, nsow))    
...
# Pre-compute construction costs across both dimensions natively
for i, dh in enumerate(delta_h_seq):
    for j, y_ev in enumerate(yr_elev_seq):
        cc_ens[i, j, :] = construction_cost(dh, sqft, y_ev, drs)

# Process SOWs via optimized vectorized dimensional expansions
chunk_size = 5000
years_arr = np.arange(201)

for start_idx in range(0, nsow, chunk_size):
    if verbose: print(f"Evaluating chunk index {start_idx}")
    end_idx = min(start_idx + chunk_size, nsow)
    ...
    # Baseline computed exactly once per chunk (independent of timing/height choice)
    ...
    for i, dh in enumerate(delta_h_seq):
        ...
        # Strategy-specific metrics computed exactly once per chunk (independent of timing choice)
        ...
        for j, y_ev in enumerate(yr_elev_seq):
            # 1. Slice damage profiles contextually at year of execution
            ead = np.hstack((ead_init[:, 0:y_ev], ead_elev[:, y_ev:201]))
            disc_ead = ead * dr_c
            led_ens[i, j, start_idx:end_idx] = np.sum(disc_ead * lifespan_mask, axis=1)
            
            # 2. Slice annual survival parameters contextually at year of execution
            p_annual = np.where(years_arr < y_ev, prob_init, prob_elev)
            p_annual = np.where(lifespan_mask, p_annual, 1.0)
            lr_ens[i, j, start_idx:end_idx] = np.prod(p_annual, axis=1)
...
baseline_led = led_ens[0, :, :] # shape: (num_years, nsow)
...
robustness_scores = np.mean(robustness_mask, axis=2) * 100 # Mean along SOW dimension
...
df_results.to_csv('data/objectives_multyrs_5e6.csv', index=False)
if verbose: print("\nResults saved to 'objectives_multyrs_5e6.csv'")
